...
const client = new OpenAI({
    ...
    apiKey: "eyJhbGciOiJFUzUxMiIsImtpZCI6IjhhNmVjNWFmLThlNWEtNDQxOS04NmM4LWRkMDkxN2U1YWNlMSIsInR5cCI6IkpXVCJ9..ALqOgOy_4Lu_iw1HWjOI5uA2KgbnUqNTQt7SZuUF60PbvnqxryWRCdBirC6Xt6IAVqGzgRxJXDVp7iqmaSqBH7OWABICXHXSn_nvR5Ii4FM2Y50QDABrqNkk4PnD2gRDar9CUZ_ZggY6xOqbWm-bx7L7gN5dQO7lE6GtmlFVS6y-y3-6",
    baseURL: "https://apps.aws-london-novaprd1.svc.singlestore.com:8000/modelasaservice/d31d82a2-1310-4c1f-a53e-642e4c4969f4/v1"
, fetch: (input, init = {}) => {
    return fetch(input, {
      ...init,
      headers: {
...
export const makeQuestionEasierHarder = async (question,difficulty) => {
    const prompt = `Make the question ${difficulty}, i only want 1 question and it can't be to far from the original question. Make sure the subject_course is the same!` + JSON.stringify(question) + grandmaIsDying;
    try{
        const response = await client.chat.completions.create({
            model: "unsloth/Meta-Llama-3.1-8B-Instruct",
            messages: [
